                        ...
                        <td class="whitespace-nowrap px-3 py-4">
                            <x-dropdown>
                                ...
                                <x-slot name="content">
                                    ...
                                    <x-dropdown-link :href="route('companies.edit', $data->id)">
                                        {{ __('Szerkeszt') }}
                                    </x-dropdown-link>
                                    <form method="POST" action="{{ route('companies.destroy', $data->id) }}">
                                        @csrf
                                        @method('delete')
                                        <x-dropdown-link :href="route('companies.destroy', $data->id)" onclick="event.preventDefault(); this.closest('form').submit();">
                                            {{ __('Töröl') }}
                                        </x-dropdown-link>
                                    </form>
                                </x-slot>
